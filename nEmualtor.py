...
def main(argv):
    emualtor_sender_udp_port = int(argv[0])
    receiver_addr = argv[1]
    receiver_port = int(argv[2])
    receiver_pair = (receiver_addr, receiver_port)
    emualtor_receiver_udp_port = int(argv[3])
    sender_addr = argv[4]
    sender_port = int(argv[5])
    sender_pair = (sender_addr, sender_port)
    pack_discard_probability = int(argv[6])
    verbose_mode = int(argv[7])
    ...
    while True:
        try:
            ...
            return "Sender Part try socket.error Occured"
        ...
        try:
            ...
            return "Receiver Part try socket.error Occured"
